...
// Ensure environment variables are clean of hidden carriage returns or spaces
...
if (process.env.GOOGLE_REDIRECT_URI) process.env.GOOGLE_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI.trim();

// Diagnostic check to ensure credentials are visible to the server runtime
...
console.log('Redirect URI:', process.env.GOOGLE_REDIRECT_URI || 'Not explicitly set in env');
console.log('-----------------------------------');
...
async function saveEnvironmentValues(values) {
  ...
  // Clean runtime process.env values instantly after saving
  if (process.env.GOOGLE_CLIENT_ID) process.env.GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID.trim();
  if (process.env.GOOGLE_CLIENT_SECRET) process.env.GOOGLE_CLIENT_SECRET = process.env.GOOGLE_CLIENT_SECRET.trim();
  if (process.env.GOOGLE_REDIRECT_URI) process.env.GOOGLE_REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI.trim();
...
app.post('/api/save-drive-config', async (request, response) => {
  ...
  if (Object.values(values).some((value) => !value) || !values.GOOGLE_REDIRECT_URI.startsWith('http')) {
...
function beginGoogleAuthorization(request, response) {
  try {
    ...
    const redirectUri = `${forwardedProtocol}://${forwardedHost}/auth/google/callback`;
...
app.get('/api/drive/files', async (request, response) => {
  try { return response.json({ success: true, files: await listExcelFiles() }); }
  catch (error) { return response.status(503).json({ success: false, error: error.message }); }
});
...
app.get('/api/drive/files/:fileId', async (request, response) => {
  ...
  } catch (error) { response.status(503).json({ success: false, error: error.message }); }
});
